...
impl SolarTime {
    ...
    /// AL-MINABR PATCH: upstream unwrapped the `Option` here and panicked whenever
    /// the sun never reaches the requested angle - which happens every summer above
    /// roughly 48° latitude. London in mid-July crashed the calculation outright.
    ///
    /// The fallback is deliberately an out-of-range time in the direction the
    /// caller clamps: Fajr is clamped upward against `safe_fajr` and Isha downward
    /// against `safe_isha`, so returning something impossibly early / impossibly
    /// late makes the existing night-portion clamp take over. That is exactly what
    /// the reference Adhan implementations do when the angle is unreachable.
    /// See vendor/salah/PATCH.md.
    pub fn time_for_solar_angle(&self, angle: Angle, after_transit: bool) -> DateTime<Utc> {
        self.try_time_for_solar_angle(angle, after_transit).unwrap_or(if after_transit {
            self.sunset + Duration::hours(24)
        } else {
            self.sunrise - Duration::hours(24)
        })
    }

    /// `None` when the sun does not reach `angle` on this date at this latitude.
    pub fn try_time_for_solar_angle(&self, angle: Angle, after_transit: bool) -> Option<DateTime<Utc>> {
