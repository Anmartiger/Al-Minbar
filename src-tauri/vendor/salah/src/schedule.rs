...
impl PrayerTimes {
    pub fn new(date: NaiveDate, coordinates: Coordinates, parameters: Parameters) -> PrayerTimes {
        ...
        let final_asr = asr
            ...
            .rounded_minute(parameters.rounding);
        // AL-MINABR PATCH: honour `parameters.maghrib_angle`. Upstream stores the
        // field, and Method::Tehran even sets it to 4.5, but nothing ever reads it -
        // Maghrib is always plain sunset. Methods that place Maghrib when the sun is
        // N degrees below the horizon (Tehran, Ja'fari) are therefore wrong upstream.
        // Uses the same solver Isha does. See vendor/salah/PATCH.md.
        let maghrib_base = if parameters.maghrib_angle > 0.0 {
            solar_time
                .try_time_for_solar_angle(Angle::new(-parameters.maghrib_angle), true)
                .unwrap_or(solar_time.sunset)
        } else {
            solar_time.sunset
        };
